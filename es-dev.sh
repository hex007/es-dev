...
SCRIPT_URL="https://raw.githubusercontent.com/hex007/es-dev/master/es-dev.sh"
SCRIPT_DIR="$(dirname "$0")"
SCRIPT_DIR="$(cd "$SCRIPT_DIR" && pwd)"
SCRIPT_NAME="$(basename "$0")"
REPO_FILE="es-repos.txt"
REPO_FILE_FULL="$SCRIPT_DIR/$REPO_FILE"
PATCH_FILE="es-patches.txt"
PATCH_FILE_FULL="$SCRIPT_DIR/$PATCH_FILE"
...
function build_menu() {
    ...
    while true; do
        ...
        setup_sources && handle_build
    done
}

function setup_sources() {
    # priori: declaration `repo`, `branch`

    # Set developer and src directory
    developer=$(echo "$repo" | sed "s#.*https://github.com/\([^/]*\)/.*#\1#")
    ...
    ( git clone --recurse-submodules --progress -b "$branch" "$repo" "$es_src_dir" 2> /dev/null ) || \
    ( git -C "$es_src_dir" pull --rebase || ( git -C "$es_src_dir" merge --abort && git -C "$es_src_dir" pull -X theirs ) ) || \
    ( dialogMsg "Failed to download ${developer}'s $branch ES branch.\n\nPlease, check your connection and try again." && return 1 )

    patches_menu || return 1
    # returns 0 on successful clone or pull or pull discarding confilcts
}

function patches_menu() {
    if [[ ! -s "$PATCH_FILE_FULL" ]]; then
        return 0
    fi

    dialogYesNo "Would you like to apply any patches from $PATCH_FILE or remove all patches applied to this repository?" || return 0
...
# Edit Repo Menu ############################################## Edit Repo Menu #

function edit_repo_menu() {
    local options=()
    local choice
    local i=1
    local repo
...
# Remove Default Menu #################################### Remove Default Menu #
